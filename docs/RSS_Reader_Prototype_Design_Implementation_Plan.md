...
- hovering over a rendered highlight for 200 milliseconds, or immediately focusing/clicking it, shows its annotation when present and permits adding, editing, removing, or deleting the annotation and navigating to Notes. Clean contextual pop-ups close on outside click; a pop-up with a new or changed unsaved annotation remains open, shakes subtly, shows `Not saved!` beside its Annotation label, and blocks article changes, view navigation, and other outside interactions until the draft is saved or explicitly discarded with the popup X (or Cancel for a new annotation). Annotated-highlight deletion is confirmed in-app;
